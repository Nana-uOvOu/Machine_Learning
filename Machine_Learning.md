...
## 逻辑回归
### Sigmoid函数
1. 对于回归任务来说，*输出的是连续预测值*；对于分类问题来说，*输出的是离散类别值*。**通过给回归任务设定一个阈值，就可以做到分类。**例如$\hat{y}>E$是恶性肿瘤，$\hat{y}\leq E$是非恶性肿瘤，其中$E$是预测均值.**阈值很难设定，容易受到离群值影响**。
...
3. 是一个无约束优化问题，可以使用
   ...
   > 1. 首先为了近似出函数$f(\theta)$在$\theta_0$上的值，对$f(\theta)$求一个二阶泰勒展开，利用*梯度和 Hessian 矩阵(表示了曲率)来求解*。![alt text](image-17.png)
   > 2. 令$f(\theta)$的二阶泰勒展开的梯度=0![alt text](image-18.png)
   > 3. 可以求出近似最优参数：![alt text](image-19.png)。然而，由于是近似的解法，不一定能直接得到最优解，需要进行迭代：![alt text](image-20.png)。
   > 4. 虽然牛顿法比梯度下降*收敛速度快*，但牛顿法*计算成本大*（尤其是在Hessian矩阵）。牛顿法收敛速度快，是因为不仅考虑了一阶梯度，还考虑了更加精细的二阶梯度。
   > 5. 牛顿法求解逻辑回归：![alt text](image-21.png)![alt text](image-22.png)![alt text](image-23.png)

### 多分类问题
![alt text](image-24.png)
1. OVO（One vs one）：*将多分类转变为多个二分类*。对于K个类别，两两训练一个分类器，共$\frac{k(k-1)}{2}$个分类器。使用时，每个二分类器进行投票，给一个类别进行投票。可能出现平票，*需要加权投票，使用输出的置信度进行相加*。
![alt text](image-25.png)
![alt text](image-26.png)
2. OVR(One vs rest)：*将多分类转变为多个二分类*。对于K个类别，每次选取一个类别作为正类，其余全是负类，训练K个二分类器。使用时选取概率最高的二分类器对应的类别输出。*可能导致类边界不一致（多个分类器都识别为正类）*，无法捕捉类间关系。
3. Softmax回归（多分类逻辑回归）：将多个类别的概率*建立在一个模型内，输出所有类别的概率分布*
   1. sigmoid只适合二分类，是独立计算的，若同时计算很多结果的sigmoid值，输出不能保证和为1。而*softmax*可以保证输入一个向量，给出一个0-1范围内的值向量，且和为1.
   2. ![alt text](image-27.png)
   3. 最大似然估计（将输出概率的$y_i$累乘）：
   > 1. ![alt text](image-29.png)
   > 2. 求导的时候要注意，每各类别都有对应的权重$w_j$，对于每个不同类别的权重，每一项的导数值不同。
...
# 第四章 感知机
感知机模型是一种线性分类模型，*可以用于解决线性可分的二分类问题*。
找到一个超平面分割所有数据点，将数据点分为两类。如果线性不可分，即无法将所有数据点使用超平面分割，则不适合。
![alt text](image-36.png)
##### 感知机分类过程
1. 输入信号向量$X$；
2. 经过神经元加权求和得到特征$Z = W^T×X + b$
3. 激活结果（使用阶跃函数）：$\hat{Y} = sign(Z)$
   1. ![alt text](image-37.png)
4. 输出分类给下一层神经元
##### 感知机的损失函数
感知机输出的是正类1，负类-1。所以目标是最大化$\sum_i^N y_i × (W^T×X_i + b)$，相同时+1，不同-1。
1. $L(w) = \sum_i^N max(0,y_i × (W^T×X_i + b))$
2. 样本分为正类则损失为0；分为负类则损失为![alt text](image-38.png)
3. 使用SGD等方法更新权重$w$和$偏置b$。
4. 本质是随机寻找一个初始超平面，接着随机对某个分类点进行修正（更新$w,b$），直到找到能正确分类的所有点。
##### 多分类感知机
1. 建立在一对多方法上。对于C个类别，训练C个感知机，区分是类别j或非类别j。确保每个样本在正确类别上的得分大于其他类别。
![alt text](image-39.png)
2. ![alt text](image-41.png)![alt text](image-40.png)
##### 感知机弱点
线性可分：逻辑与/逻辑或都可以区分出来；而线性不可分（异或）时，感知机是不可以区分的。
![alt text](image-43.png)
![alt text](image-44.png)
![alt text](image-45.png)

# 第五章 神经网络
1. 也称为Connectionism (联结主义)学习:通过大量的连接节点（神经元）之间的相互作用来习和处理信息。在这种模型中，信息通过这些连接（即神经元之间的权重）进行传递和加工